...
|----|-----------|-----|----|
| VM101 | k3s worker | 4GB | 192.168.1.30 |
...
Автозапуск основных сервисных VM задан через Terraform: VM101–VM104 имеют `on_boot = true`. VM105 запускается вручную по необходимости. Порядок зависимого запуска через Proxmox `startup` намеренно не используется из-за требований к расширенным системным правам.
