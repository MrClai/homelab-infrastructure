...
При диагностике инцидента 3 значения `AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY` были вставлены в рабочий чат в открытом виде. Для homelab не критично, но по тому же принципу, что применялся к `gitlab-ci-key.json` (см. журнал сессии 23.07 в `roadmap`) — растрата в чате считается компрометацией. Ключ подлежит ротации через `mc admin accesskey`, независимо от вероятности реального использования.
