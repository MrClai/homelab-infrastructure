...
`ansible.cfg` и roadmap заявляли, что подключение выполняется отдельным
сервисным пользователем `ansible` (создан `create-ansible-user.yml`, задача 9.7),
а `clai` остаётся только для ручной админки. Запись от 23.07.2026 в roadmap
отмечала это как проверенное: `host_key_checking = True` включён, fingerprints
...
на реальном SSH-подключении. Поэтому `ansible -m ping`, зафиксированный как
✅ Done в roadmap 23.07, был пройден — но под `clai` с оверрайдом из inventory,
...
Тест конфигурации подключения был бы частью CI (P0.2 — линтинг и smoke-test
